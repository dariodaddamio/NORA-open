...
from process_link import (
    ...
    format_eta_remaining_label,
    format_pipeline_failure,
...
SAVE_MAX_CONCURRENT = max(1, int(os.getenv("SAVE_MAX_CONCURRENT", "2")))
...
_save_semaphore = asyncio.Semaphore(SAVE_MAX_CONCURRENT)
        ...
        async with lock:
            async with _save_semaphore:
                try:
                    ...
                    await interaction.followup.send(f"Forced processing failed: {format_pipeline_failure(e)}")
...
async def save(interaction: discord.Interaction, url: str):
    ...
    async with _save_semaphore:
        ...
        async def _eta_updater() -> None:
            while not stop_updater.is_set():
                ...
                if snap:
                    ...
                    snap_status = str(snap.get("status") or "running")
                else:
                    ...
                    snap_status = "running"
                remaining_label = format_eta_remaining_label(
                    remaining_seconds=remaining,
                    elapsed_seconds=elapsed_runtime,
                ...
                try:
                    await status_message.edit(
                        content=(
                            "🧠 NORA is thinking...\n"
                            f"🎬 Video length: `{_format_duration(video_duration)}`\n"
                            f"⏱️ ETA remaining: `{remaining_label}`\n"
        ...
        try:
            ...
            await interaction.followup.send(
                f"Failed to process link: {format_pipeline_failure(e)}\n"
