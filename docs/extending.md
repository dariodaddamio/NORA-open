...
| Categories and tag rules | `taxonomy.json` (copy from `taxonomy.example.json` for a starter file) | Your vocabulary and synonyms; `TAXONOMY_MODE=auto` appends new categories |
...
- `taxonomy.example.json` — starter taxonomy template; copy to `taxonomy.json` to customize
- `taxonomy.json` — your categories (optional; built-in defaults if absent; keep local and out of git)
