...
- **True video understanding** (model looks at pixels) is behind **`VISION_CONTEXT_ENABLED`** (default off). When enabled, selected frames are sent to `OPENROUTER_VISION_MODEL` via OpenRouter. The off path stays OCR-text-only.
